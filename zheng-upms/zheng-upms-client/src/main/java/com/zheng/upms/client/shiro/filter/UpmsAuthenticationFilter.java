...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        if ("server".equals(upmsType)) {// isAuthenticated/isRemembered是互斥的，即如果其中一个返回true，另一个返回false
            return subject.isAuthenticated(); // 里面调用的是onPreHandle，而里面调用的是onPreHandle会自动调用这两个方法决定是否继续处理 isAccessAllowed onAccessDenied
        }
        return false;
    }
    // 需要登录会先跳转到这个页面
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer ssoServerUrl = new StringBuffer(PropertiesFileUtil.getInstance("zheng-upms-client").get("zheng.upms.sso.server.url"));//获取到登录的Url http://upms.zhangshuzheng.cn:1111
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("zheng-upms-client").get("zheng.upms.type");
        if ("server".equals(upmsType)) { // 跳转到http://upms.zhangshuzheng.cn:1111/sso/login登录页面
            WebUtils.toHttp(response).sendRedirect(ssoServerUrl.append("/sso/login").toString()); // 这里重定向到了sso模块，回去调用sso的login方法
            return false;
        }
